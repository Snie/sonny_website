...
describe("AcademicSection", () => {
	it("renders heading 'Academic Work'", () => {
		...
		expect(screen.getByRole("heading", { name: "Academic Work" })).toBeInTheDocument();
	});

	it("renders entry titles", () => {
		render(<AcademicSection />);
		expect(
			screen.getByText("M.Sc. Thesis — Human-like Controllers for Simulated Cars"),
		).toBeInTheDocument();
		expect(
			screen.getByText(
	...
	it("renders institutions", () => {
		...
		expect(screen.getByText("Dolphin Engineering Sagl")).toBeInTheDocument();
	});
});
